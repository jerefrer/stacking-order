...
class StackingOrderTest < Minitest::Test
  ...
  def test_2_entries_1_row_2_columns
    assert_equal [1, 2], stacking_order(entries: 2, rows: 1, columns: 2)
  end

  def test_3_entries_1_row_2_columns
    assert_equal [1, 3, 2], stacking_order(entries: 3, rows: 1, columns: 2)
  end

  def test_5_entries_1_row_2_columns
    assert_equal [1, 4, 2, 5, 3], stacking_order(entries: 5, rows: 1, columns: 2)
  end

  def test_6_entries_1_row_2_columns
    assert_equal [1, 4, 2, 5, 3, 6], stacking_order(entries: 6, rows: 1, columns: 2)
  end

  def test_6_entries_2_rows_2_columns
    assert_equal [1, 3, 5, nil, 2, 4, 6], stacking_order(entries: 6, rows: 2, columns: 2)
  end

  def test_13_entries_2_rows_2_columns
    assert_equal [1, 5, 9, 13, 2, 6, 10, nil, 3, 7, 11, nil, 4, 8, 12],
                 stacking_order(entries: 13, rows: 2, columns: 2)
  end

  # Edge cases
  def test_1_entry_1_row_1_column
    assert_equal [1], stacking_order(entries: 1, rows: 1, columns: 1)
  end

  def test_0_entries_1_row_1_column
    assert_equal [], stacking_order(entries: 0, rows: 1, columns: 1)
  end

  def test_exactly_fills_one_page
    # 4 entries perfectly fill one 2x2 grid
    assert_equal [1, 2, 3, 4], stacking_order(entries: 4, rows: 2, columns: 2)
  end

  def test_exactly_fills_multiple_pages
    # 8 entries perfectly fill two 2x2 grids
    assert_equal [1, 3, 5, 7, 2, 4, 6, 8], stacking_order(entries: 8, rows: 2, columns: 2)
  end

  # Test with more columns
  def test_1_row_3_columns
    assert_equal [1, 4, 7, 2, 5, 8, 3, 6, 9], stacking_order(entries: 9, rows: 1, columns: 3)
  end

  def test_5_entries_1_row_3_columns
    assert_equal [1, 3, 5, 2, 4], stacking_order(entries: 5, rows: 1, columns: 3)
  end

  # Test with more rows
  def test_3_rows_1_column
    assert_equal [1, 2, 3], stacking_order(entries: 3, rows: 3, columns: 1)
  end

  def test_5_entries_3_rows_1_column
    # 5 entries in 3x1 grid = 2 pages
    # Cell 0: entries 1,2; Cell 1: entries 3,4; Cell 2: entry 5
    assert_equal [1, 3, 5, 2, 4], stacking_order(entries: 5, rows: 3, columns: 1)
  end
  ...
  def test_15_entries_2_rows_2_columns
    ...
    assert_equal [1, 5, 9, 13, 2, 6, 10, 14, 3, 7, 11, 15, 4, 8, 12],
                 stacking_order(entries: 15, rows: 2, columns: 2)
  end
  ...
  def test_2_entries_2_rows_2_columns
    ...
    assert_equal [1, 2],
                 stacking_order(entries: 2, rows: 2, columns: 2)
  end
  ...
  def test_895_entries_7_rows_3_columns
    ...
    result = stacking_order(entries: 895, rows: 7, columns: 3)
    cells_per_page = 21
